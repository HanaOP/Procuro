...
export default function ManagerDashboard() {
  const { user } = useAuth()
  const [stats, setStats] = useState({ pending: 0, high: 0, approved: 0, exceptions: 0 })
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.allSettled([
      getPendingRequests(), getHighPriority(), getApprovedList(), getExceptions()
    ]).then(([pending, high, approved, exc]) => {
      setStats({
      ...
      <div className="space-y-8">
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: 'Pending Review',  value: stats.pending,    path: '/manager/pending',       alert: stats.pending > 0 },
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            <Link to="/manager/pending" className="btn-primary">Review Pending</Link>
